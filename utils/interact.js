...
const contract = require("../artifacts/contracts/ComicsApeSocialClub.sol/ComicsApeSocialClub.json");
...
export const getPreprice = async () => {
  const preprice = await nftContract.methods.preprice().call();
  return preprice;
};
...
export const presaleMint = async (mintAmount) => {
  ...
  const tx = {
    ...
    value: parseInt(
      web3.utils.toWei(String(config.preprice * mintAmount), "ether")
    ).toString(16), // hex
    gas: String(50000 * mintAmount),
